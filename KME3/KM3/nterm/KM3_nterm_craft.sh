#!/bin/bash
#SBATCH --job-name=KM3_nterm_craft
...
CRAFT_WD="/path/to/project/KM3/nterm"
CRAFT_CONFIG="/path/to/project/config_nterm.yaml"
...
echo "[$(date '+%H:%M:%S')] Phase 2a -- geometry optimisation (KM3_nterm_opt.com)"
g16 < "$CRAFT_WD/KM3_nterm_opt.com" > "$CRAFT_WD/KM3_nterm_opt.log"
...
craft-hf-input "$CRAFT_WD/KM3_nterm_opt.log" --config "$CRAFT_CONFIG"
[ $? -ne 0 ] && echo "ERROR in Phase 2b (craft-hf-input)" && exit 1

# -- Phase 2c: HF/6-31G(d) single-point for ESP/RESP -------------------------
echo "[$(date '+%H:%M:%S')] Phase 2c -- HF single-point (KM3_nterm_hf.com)"
g16 < "$CRAFT_WD/KM3_nterm_hf.com" > "$CRAFT_WD/KM3_nterm_hf.log"
[ $? -ne 0 ] && echo "ERROR in Phase 2c (Gaussian HF)" && exit 1

# -- Phase 3: AMBER parameterization ------------------------------------------
echo "[$(date '+%H:%M:%S')] Phase 3 -- craft-amber"
craft-amber "$CRAFT_WD/KM3_nterm_hf.log" --config "$CRAFT_CONFIG"
