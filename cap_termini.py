...
Atom naming matches AMBER ff14SB conventions (verified against MEO reference).

Usage:
    python cap_termini.py KME3.pdb               # writes KME3_capped.pdb
    python cap_termini.py KME3.pdb output.pdb
"""
...
def inspect_termini(atoms, N_pos, C_pos):
    ...
    Returns
    -------
    h_on_N   : list of atom indices — H atoms bonded to backbone N (to remove)
    oxt_idx  : list of atom indices — extra carboxylate O on C (to remove)
...
def cap(input_pdb, output_pdb):
    ...
    # Collect first-occurrence position for each atom name
    pos = {}
    for a in atoms:
        pos.setdefault(a['name'], np.array([a['x'], a['y'], a['z']]))

    # Verify required backbone atoms are present
    ...
    # ACE (resSeq 1): H1 CH3 H2 H3 C O
    for name, xyz in [('H1', ACE_Hs[0]), ('CH3', ACE_CH3),
                      ('H2', ACE_Hs[1]), ('H3',  ACE_Hs[2]),
                      ('C',  ACE_C),     ('O',   ACE_O)]:
        out.append(pdb_line(ser, name, 'ACE', 1, xyz)); ser += 1

    # Residue (resSeq 2): write N first, inject new amide H, then the rest
    n_written = 0
    for i, a in enumerate(atoms):
        if i in remove_idx:
            continue
